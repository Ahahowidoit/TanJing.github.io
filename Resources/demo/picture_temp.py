...
def get_images_from_folder(folder_path, save_path):
    i = 1
    for file_name in os.listdir(folder_path): #遍历文件夹里的文件
        print(file_name)
        if not file_name.endswith('.xlsx'): # 如果文件不是excel则跳过
            continue

        # 通过连接文件夹路径和文件名来获取完整的文件路径，并使用openpyxl库的load_workbook() 函数加载Excel文件，
        # 然后通过wb.active 选择活动工作表
        ...
        for row in ws.iter_rows():
            ...
            for cell in row:
                ...
                # img_data = requests.get(url1+'?x-oss-process=image/resize,m_fixed,h_100,w_100')
                # img_data = requests.get(url1)
                # img_path = os.path.join(save_path, f"{cell.value}.jpg") # 给图片用条码命名
                img_path = os.path.join(save_path, f"{cert_no}.jpg") # 给图片用质检证书号命名
